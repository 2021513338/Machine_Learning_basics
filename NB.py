...
def NB_model(X, y, n_splits, n_repeats, random_state=None):
    accuracies = []
    aucs = []
    fold_results = []
    fold_indices = []
    ...
    for repeat in range(n_repeats):
        ...
        for train_index, test_index in kf.split(X):
            ...
            y_pred_decision = nb_classifier.predict_proba(X_test)[:, 1]
...
accuracies, aucs, fold_results, fold_indices = NB_model(X, y, n_splits=10, n_repeats=100)
save_results_to_file(accuracies, aucs, fold_results, fold_indices, 'nb_homo_DC_pDC.txt')